...
import {
  Target, Shield, AlertTriangle, TrendingUp, TrendingDown, Zap, Unlock,
  Ruler, Scale, Loader2, Radio, Swords, ClipboardList, type LucideIcon,
} from "lucide-react";
...
export default function OpponentPage() {
  ...
  if (myForm && oppForm) {
    ...
    if (mA > oD + 8) {
      tips.push({ icon: Target, text: `Offensive Advantage — ${myTeam} (Attack: ${mA}) holds a strong edge over ${oppTeam}'s defensive structure (Defence: ${oD}). Exploit space behind their fullbacks early with direct wide passes.` });
    } else if (mA < oD - 5) {
      tips.push({ icon: Shield, text: `Compact Backline Ahead — ${oppTeam}'s defence (${oD}) is disciplined. Work through controlled buildup, set pieces, and second balls around the 18-yard box.` });
    }

    if (oA > mD + 8) {
      tips.push({ icon: AlertTriangle, text: `Defensive Threat — ${oppTeam}'s attacking rating (${oA}) poses danger against our defence (${mD}). Maintain a compact mid-block and double-cover their primary wingers.` });
    } else if (mD > oA + 5) {
      tips.push({ icon: Shield, text: `Defensive Superiority — ${myTeam}'s backline (${mD}) comfortably covers ${oppTeam}'s goal threat (${oA}). Fullbacks are free to support high line overlaps.` });
    }

    // Form breakdown (strictly out of last 5 matches)
    if (myW >= 4) {
      tips.push({ icon: TrendingUp, text: `High Momentum — ${myTeam} have won ${myW} of their last ${totalMy} matches in BSD fixtures. Squad confidence is high; maintain current tactical rhythm.` });
    }
    if (oppW >= 3) {
      tips.push({ icon: TrendingDown, text: `Opponent Form — ${oppTeam} have won ${oppW} of their last ${totalOpp} matches (scoring ${oppScored.toFixed(1)} goals/game). Exercise discipline in early transitions.` });
    } else if (oppL >= 3) {
      tips.push({ icon: Zap, text: `Vulnerable Form — ${oppTeam} have lost ${oppL} of their last ${totalOpp} matches. Press them high from kickoff to force defensive errors.` });
    }

    if (oppConceded >= 1.6) {
      tips.push({ icon: Unlock, text: `Defensive Leakage — ${oppTeam} have been conceding ${oppConceded.toFixed(1)} goals per match in recent fixtures. Test their goalkeeper with early shots.` });
    }

    if (oppForm.best_formation && myForm.best_formation) {
      tips.push({ icon: Ruler, text: `Tactical Setup — ${oppTeam} frequently operate in a ${oppForm.best_formation} shape. Matching or countering with ${myTeam}'s preferred ${myForm.best_formation} provides optimal midfield coverage.` });
    }

    if (!tips.length) {
      tips.push({ icon: Scale, text: `Balanced Matchup — ${myTeam} and ${oppTeam} present closely matched ratings (${mA}/${mD} vs ${oA}/${oD}). Midfield execution and set-piece efficiency will decide the result.` });
    }
  }
  ...
  return (
    <div className="max-w-screen-xl mx-auto px-5 py-10 space-y-6">
      <div>
        <p className="section-label mb-2 flex items-center gap-1.5"><ClipboardList size={13} /> Module 2</p>
      ...
      <button onClick={run} disabled={loading} className="btn-volt w-full py-4 text-base flex items-center justify-center gap-2">
        {loading ? <><Loader2 size={14} className="animate-spin inline" /> Fetching…</> : <><Radio size={14} className="inline mr-1" />Get Scout Report</>}
      </button>

      {myForm && oppForm && (
        <>
          <div className="card">
            <p className="section-label mb-5 flex items-center gap-1.5"><Swords size={13} /> Head-to-Head</p>
            <div className="grid grid-cols-3 gap-4 text-center">
              <div>
                <p className="font-mono text-[10px] text-mt uppercase tracking-wider mb-3">{myTeam}</p>
                <div className="space-y-2">
                  {([["Attack", myForm.attack, true, Swords], ["Defence", myForm.defence, false, Shield]] as const).map(([lbl, val, isAtt, Icon]) => (
                    <div key={String(lbl)} className={`rounded-xl p-3 border ${isAtt ? "bg-volt/8 border-volt/20" : "bg-cyan/8 border-cyan/20"}`}>
                      <p className="text-[10px] text-mt mb-0.5 flex items-center gap-1"><Icon size={10} /> {String(lbl)}</p>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {([["Attack", oppForm.attack, true, Swords], ["Defence", oppForm.defence, false, Shield]] as const).map(([lbl, val, isAtt, Icon]) => (
          ...
          <div className="card">
            <p className="section-label mb-4 flex items-center gap-1.5"><ClipboardList size={13} /> AI Scout Report</p>
            <div className="space-y-3">
              {tips.map((t, i) => (
                <div key={i} className="bg-bg border-l-2 border-l-volt rounded-xl px-4 py-3 text-sm text-white leading-relaxed flex items-start gap-2.5">
                  <t.icon size={16} className="text-volt flex-shrink-0 mt-0.5" />
                  <span>{t.text}</span>
                </div>
              ))}
            </div>
